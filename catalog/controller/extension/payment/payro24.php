...
class ControllerExtensionPaymentpayro24 extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->load->view('extension/payment/payro24', $data);
    }
    ...
    public function confirm()
    {
        ...
        $api_key = $this->config->get('payment_payro24_api_key');
        $sandbox = $this->config->get('payment_payro24_sandbox') == 'yes' ? 'true' : 'false';
        $amount = $this->correctAmount($order_info);

        $desc = $this->language->get('text_order_no') . $order_info['order_id'];
        $callback = $this->url->link('extension/payment/payro24/callback', '', true);
    ...
    public function callback()
    {
        if ($this->session->data['payment_method']['code'] == 'payro24') {
            ...
            $this->document->setTitle($this->config->get('payment_payro24_title'));

            $data['heading_title'] = $this->config->get('payment_payro24_title');
            $data['peyment_result'] = "";
            ...
            $data['breadcrumbs'][] = array(
                'text' => $this->config->get('payment_payro24_title'),
                'href' => $this->url->link('extension/payment/payro24/callback', '', true)
            );
                    ...
                    if ($status != 10) {
                        ...
                        if ($http_status != 200) {
                            ...
                            if (empty($verify_status) || empty($verify_track_id) || empty($verify_amount) || $verify_amount != $amount || $verify_status < 100) {
                                ...
                                $config_successful_payment_status = $this->config->get('payment_payro24_order_status_id');
            ...
            $this->response->setOutput($this->load->view('extension/payment/payro24_confirm', $data));
    ...
    {
        return str_replace(["{track_id}", "{order_id}"], [$track_id, $order_id], $this->config->get('payment_payro24_success_massage'));
    }
    ...
    public function payro24_get_failed_message($track_id, $order_id, $msgNumber = null)
    {
        ...
        return str_replace(["{track_id}", "{order_id}"], [$track_id, $order_id], $this->config->get('payment_payro24_failed_massage')) . "<br>" . "$msg";
    }
